...
export const FLabel = ({ label, children, span2 }) => (
    <div className={`flex flex-col gap-1.5 ${span2 ? 'col-span-1 sm:col-span-2' : ''}`}>
        <label className="text-[10px] font-bold text-gray-400 tracking-[1.2px] font-['Outfit',_sans-serif] uppercase">{label}</label>
        {children}
    </div>
);
...
export const Txt = (props) => (
    <textarea {...props} className={`${inpClass} resize-y min-h-[82px] ${props.className || ''}`} />
);
...
export const DeleteModal = ({ label, onConfirm, onClose, isLoading }) => (
    <Overlay onClose={onClose}>
        <div className="px-8 py-10 text-center">
            ...
            <h2 className="font-['Playfair_Display',_serif] text-[19px] text-[#0c0e1a] mb-2.5 font-bold">Confirm Delete</h2>
