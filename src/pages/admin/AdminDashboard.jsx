...
import {
    LogOut, Users, FileText, X, CheckCircle,
    AlertCircle, Calendar, Camera
} from "lucide-react";
    ...
    return (
        <div className="bg-gradient-to-br from-[#f8f5ee] via-[#fdfcf9] to-[#f2f4fa] min-h-screen pt-20 pb-16 px-4 sm:px-6 relative overflow-x-hidden font-['Outfit',_sans-serif]">
            ...
            <div className="max-w-[1400px] mx-auto relative z-10">
                ...
                <AnimatePresence>
                    {error && (
                        <motion.div className="px-4 py-3 rounded-xl mb-5 text-sm flex items-center gap-2.5 font-['Outfit',_sans-serif] font-medium bg-red-50 border border-red-200 text-red-800 shadow-sm"
                            initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                            <AlertCircle size={16} className="shrink-0" />
                            <span className="flex-1">{error}</span>
                            <button onClick={() => setError("")} className="bg-transparent border-none cursor-pointer text-red-500 hover:text-red-700"><X size={14} /></button>
                        </motion.div>
                    )}
                    {success && (
                        <motion.div className="px-4 py-3 rounded-xl mb-5 text-sm flex items-center gap-2.5 font-['Outfit',_sans-serif] font-medium bg-emerald-50 border border-emerald-200 text-emerald-800 shadow-sm"
                            initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                ...
                {/* Stat Cards */}
                <motion.div className="grid grid-cols-[repeat(auto-fit,minmax(190px,1fr))] gap-4 mb-8" variants={cv} initial="hidden" animate="visible">
                ...
                <AnimatePresence mode="wait">
                    ...
                    {activeTab === "alumni" && (
                        <motion.div key="alumni" variants={iv} initial="hidden" animate="visible" exit={{ opacity: 0 }}>
                            <AlumniTab alumniList={alumniList} setSelectedItem={setSelectedItem} />
                        </motion.div>
                    )}
                    {activeTab === "donations" && (
                        <motion.div key="donations" variants={iv} initial="hidden" animate="visible" exit={{ opacity: 0 }}>
                            <DonationsTab donationList={donationList} setSelectedItem={setSelectedItem} />
                        </motion.div>
                    )}
                </AnimatePresence>
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                            ...
                            <motion.div initial={{ scale: 0.92, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.92, y: 10 }}
                                ...
                                {selectedItem.firstName ? (
                                    <>
                                        <h2 className="font-['Playfair_Display',_serif] text-[24px] font-extrabold text-[#0c0e1a] mb-6">{selectedItem.firstName} {selectedItem.lastName}</h2>
                                        ...
                                        <div className="flex gap-2.5 pt-6 border-t border-slate-100 flex-wrap font-['Outfit',_sans-serif]">
                                            {!selectedItem.isApproved && <button onClick={() => handleApprove(selectedItem._id)} className="flex-1 py-3 px-4 border-none rounded-xl bg-emerald-50 text-emerald-700 font-bold cursor-pointer flex items-center justify-center gap-2 hover:bg-emerald-100 transition-colors text-sm shadow-sm border border-emerald-100"><CheckCircle size={15} />Approve Alumni</button>}
                                            {selectedItem.isApproved && !selectedItem.isAdmin && <button onClick={() => handleMakeAdmin(selectedItem._id)} className="flex-1 py-3 px-4 border-none rounded-xl bg-sky-50 text-sky-700 font-bold cursor-pointer hover:bg-sky-100 transition-colors text-sm shadow-sm border border-sky-100">Make Admin</button>}
                                            <button onClick={() => setSelectedItem(null)} className="px-6 py-3 border border-slate-200 rounded-xl bg-white text-gray-500 font-bold cursor-pointer hover:bg-slate-50 transition-colors text-sm shadow-sm">Close</button>
                                        </div>
                                    </>
                                ) : (
                                    <>
                                        <h2 className="font-['Playfair_Display',_serif] text-[24px] font-extrabold text-[#0c0e1a] mb-6">Donation Details</h2>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
                                            {[{ l: "Amount", v: `${selectedItem.currency === "INR" ? "₹" : "$"}${selectedItem.amount}` }, { l: "Status", v: selectedItem.status }, { l: "Date", v: new Date(selectedItem.donatedAt).toLocaleString() }, { l: "Payment Method", v: selectedItem.paymentMethod }].map(it => (
                                                <div key={it.l} className="p-3.5 bg-slate-50 rounded-xl border-l-2 border-emerald-500 shadow-sm border border-slate-100 border-l-emerald-500">
                                                    <div className="text-[10px] text-gray-400 font-bold uppercase tracking-wide font-['Outfit',_sans-serif]">{it.l}</div>
                                                    <div className="text-[15px] text-[#0c0e1a] font-bold mt-1 font-['Outfit',_sans-serif]">{it.v}</div>
                                                </div>
                                            ))}
                                        </div>
                                        <button onClick={() => setSelectedItem(null)} className="px-6 py-3 border border-slate-200 rounded-xl bg-white text-gray-500 font-bold cursor-pointer hover:bg-slate-50 transition-colors text-sm font-['Outfit',_sans-serif] shadow-sm">Close</button>
                                    </>
                                )}
                            </motion.div>
                        </motion.div>
