...
import {
  Search, Filter, X, Mail, Linkedin,
  MapPin, Briefcase, GraduationCap, Users, AlertCircle,
} from "lucide-react";
...
const AlumniCard = ({ alumnus }) => {
  const initials = `${alumnus.firstName?.charAt(0) ?? ""}${alumnus.lastName?.charAt(0) ?? ""}`.toUpperCase();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-4 px-5 pt-5 pb-0">
        <div className={`flex-shrink-0 w-14 h-14 rounded-xl bg-gradient-to-br ${grad} flex items-center justify-center text-white text-lg font-extrabold shadow-md select-none`}>
          {initials || "?"}
        </div>
        <div className="min-w-0 flex-1 pt-0.5">
          ...
          <p className="text-xs text-slate-400 mt-0.5 truncate">{alumnus.email}</p>
          {alumnus.isApproved && (
      ...
      <div className="flex-1 px-5 py-4 flex flex-col gap-3">
        ...
        {(alumnus.department || alumnus.graduationYear) && (
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Department & Year</p>
              <p className="text-sm font-semibold text-slate-700 mt-0.5">
                {alumnus.department}
                {alumnus.department && alumnus.graduationYear && <span className="text-slate-300 mx-1.5">•</span>}
        ...
        {alumnus.currentCompany && (
          <div className="flex items-start gap-2.5">
            ...
            <div>
              ...
              {alumnus.jobTitle && (
                <p className="text-sm font-bold text-indigo-600 mt-0.5">{alumnus.jobTitle}</p>
              )}
              <p className="text-xs text-slate-600 font-medium">{alumnus.currentCompany}</p>
            </div>
          </div>
        )}
...
const AlumniDirectory = () => {
  ...
  const [alumniData, setAlumniData] = useState({ alumni: [], departments: [], years: [] });
  ...
  useEffect(() => {
    const loadAlumni = async () => {
      try {
        ...
        setAlumniData({
          alumni,
          departments: [...new Set(alumni.map((a) => a.department))].filter(Boolean).sort(),
          years:       [...new Set(alumni.map((a) => a.graduationYear))].sort((a, b) => b - a),
        });
      } catch (err) {
  ...
  const filteredAlumni = useMemo(() => {
    ...
    if (filterYear)       list = list.filter((a) => Number(a.graduationYear) === Number(filterYear));
    return list;
  }, [searchTerm, filterDepartment, filterYear, alumniData.alumni]);

  const hasFilters = searchTerm || filterDepartment || filterYear;
  const handleClearFilters = useCallback(() => {
    setSearchTerm(""); setFilterDepartment(""); setFilterYear("");
  }, []);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/40 pt-24 pb-16 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        ...
        {!loading && !error && (
          ...
          >
            {[
              { label: "Total Alumni",  value: alumniData.alumni.length,      icon: Users },
              { label: "Departments",   value: alumniData.departments.length,  icon: GraduationCap },
              { label: "Batch Years",   value: alumniData.years.length,        icon: Briefcase },
            ].map(({ label, value, icon: Icon }) => (
              <div key={label} className="flex items-center gap-3 bg-white rounded-2xl border border-slate-100 shadow-sm px-5 py-3.5">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 text-slate-400">
              <Filter size={15} />
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider hidden sm:block">Filter</span>
            </div>

            <select value={filterDepartment} onChange={(e) => setFilterDepartment(e.target.value)} className={selectCls}>
              <option value="">All Departments</option>
              {alumniData.departments.map((d) => <option key={d} value={d}>{d}</option>)}
            </select>

            <select value={filterYear} onChange={(e) => setFilterYear(e.target.value)} className={selectCls}>
              <option value="">All Graduation Years</option>
              {alumniData.years.map((y) => <option key={y} value={y}>{y}</option>)}
            </select>
            ...
            {!loading && !error && (
              <p className="ml-auto text-sm text-slate-500 font-medium">
                <span className="text-indigo-600 font-extrabold">{filteredAlumni.length}</span> alumni
                {searchTerm && <span className="text-slate-400"> for "{searchTerm}"</span>}
              </p>
            )}
          </div>
        </motion.div>
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-slate-400 text-sm font-medium">Loading alumni directory…</p>
          </div>
        )}
        ...
        {!loading && !error && filteredAlumni.length === 0 && (
          ...
          >
            ...
            <h3 className="text-lg font-bold text-slate-700 mb-2">No alumni found</h3>
            <p className="text-slate-400 text-sm max-w-xs">
              No results match your current filters. Try adjusting your search or clearing the filters.
            </p>
            {hasFilters && (
